...
public final class ServerRequestChannel<Type> {
    ...
    public ServerRequestChannel(PacketPipeline pipeline, Streamer<Type> streamer, Acceptor<Type> clientManager,
        PlayerAcceptor<Type> manager) {
        // this crashes if the channel cannot be created.
        // We're goin' down anyway if that happens, better now than later.
        channel = pipeline.registerChannel(new PassingChannel<Type>(clientManager, manager, streamer))
            .iterator()
            .next(); // the channel comes out in a Maybe; this extracts it
    }
