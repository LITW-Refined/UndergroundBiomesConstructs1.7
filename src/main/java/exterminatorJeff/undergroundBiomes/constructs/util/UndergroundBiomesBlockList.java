...
public class UndergroundBiomesBlockList {
    ...
    private UndergroundBiomesBlock[] definitions() {
        ...
        int i = 0;
        for (int j = 0; j < 8; j++) {
            result[i] = new UndergroundBiomesBlock(metamorphicStone, j, i);
            i++;
        }
        for (int j = 0; j < 8; j++) {
            result[i] = new UndergroundBiomesBlock(metamorphicCobblestone, j, i);
            i++;
        }
        for (int j = 0; j < 8; j++) {
            result[i] = new UndergroundBiomesBlock(metamorphicStoneBrick, j, i);
            i++;
        }
        for (int j = 0; j < 8; j++) {
            result[i] = new UndergroundBiomesBlock(igneousStone, j, i);
            i++;
        }
        for (int j = 0; j < 8; j++) {
            result[i] = new UndergroundBiomesBlock(igneousCobblestone, j, i);
            i++;
        }
        for (int j = 0; j < 8; j++) {
            result[i] = new UndergroundBiomesBlock(igneousStoneBrick, j, i);
            i++;
        }
        for (int j = 0; j < 8; j++) {
            result[i] = new UndergroundBiomesBlock(sedimentaryStone, j, i);
            i++;
        }
        return result;
    }
    ...
    public static BlockMetadataBase blockMetadataBaseNamed(String name) {
        return (BlockMetadataBase) (Block.getBlockFromName(name));
    }
